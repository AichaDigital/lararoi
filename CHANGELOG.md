...
### Fixed
- Preflight hardening after the post-ship adversarial review of v1.0.3 (AID-325): a drop of a pre-existing `vat_verifications` table now demands DOUBLE proof — the physical index fingerprint (the UNIQUE composite without the plain one, matched structurally so database prefixes cannot skew the verdict) AND larabill's ledger row, which corroborates but is never sufficient alone (a copied/squashed/stale ledger must never cost a table; a re-timestamped published lararoi table must never be claimed as "legacy"). The ledger is read and cleaned through Laravel's official `migration.repository` (honors custom migrations tables, connections and repository bindings). The abort path gains an explicit operator escape hatch — `lararoi.upgrade.assume_legacy_vat_table` / env `LARAROI_ASSUME_LEGACY_VAT_TABLE` — a human decision taken after verifying the table and exporting it (the packaged guidance now recommends a pre-migrate export: legacy rows may hold residual evidence value from raw VIES responses). Operational caveats (`migrate --pretend` blindness, deliberate drop-before-ledger-cleanup ordering, non-restoring `down()`) are documented in the migration itself. A second adversarial pass over the fix hardened two more spots: the hatch parses strictly (`filter_var` boolean — a published config carrying the string `'false'`/`'off'` never arms the drop) and the mid-upgrade shortcut demands the FULL lararoi create column set besides the plain composite index (a stale/copied canonical create row + an app table that merely has the index can no longer reach the rename). A third pass tightened the shortcut's index-set proof: the legacy UNIQUE composite must be ABSENT (lararoi's create never produces one; column names alone cannot tell the schemas apart, since legacy larabill carries the same 14 names), so a table carrying both composites under a copied create row aborts instead of reaching the rename.
